...
//    CGFloat statusWidth = self.mj_width;
    ...
    CGFloat statusWidth = frame.size.width;
    self.circleView.center = CGPointMake(self.centerX - statusWidth/2 + 25/2, self.mj_height * 0.5);
    self.statusLabel.frame = CGRectMake(self.circleView.right+10, self.circleView.y-5, statusWidth, statusHeight);
}
...
- (void)adjustStateWithContentOffset
{
    ...
    if (self.scrollView.isDragging) {
        ...
        float moveY = currentOffsetY - happenOffsetY;
        if (moveY > 65)
            moveY = 65;
        self.circleView.progress = (moveY-15) / (65-15);
