...
{
    if (!_circleView) {
        _circleView = [[CircleView alloc] initWithFrame:CGRectZero];
        _circleView.bounds = CGRectMake(0, 0, 25, 25);
        _circleView.autoresizingMask  = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
...
//    self.activityView.center = self.arrowImage.center;
    ...
    CGRect frame = [text boundingRectWithSize:CGSizeMake(0, 20) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:self.statusLabel.font} context:nil];
    self.statusLabel.width = frame.size.width;
    self.statusLabel.centerX = self.centerX + 25;
    //3.圆
    self.circleView.center = CGPointMake((self.mj_width-self.statusLabel.bounds.size.width) * 0.5 + 25/2, self.mj_height * 0.5+15);
}
...
- (void)settingLabelText
{
    ...
    CGRect frame = [text boundingRectWithSize:CGSizeMake(0, 20) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:self.statusLabel.font} context:nil];
    self.statusLabel.width = frame.size.width;
    self.statusLabel.centerX = self.centerX + 25/2;
		...
		case MJRefreshStateNormal: // 普通状态
        {
            if (self.state == MJRefreshStateRefreshing) {
                [UIView animateWithDuration:MJRefreshSlowAnimationDuration * 0.6 animations:^{
                    ...
                    //移除动画
                    [_circleView.layer removeAnimationForKey:@"rotateAnimation"];
